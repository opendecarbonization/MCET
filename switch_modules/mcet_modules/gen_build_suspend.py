"""
Use this module instead of switch_model.generators.core.build to add the
capability to retire plants before they reach gen_max_age years old.

This module defines all the same elements as the standard
switch_model.generators.core.build, but also defines a new SuspendGen variable
that shows how much capacity of each generation project from each previous
build year should be suspended/retired. This variable is also reported in the
gen_cap.csv output file. SuspendGen also reduces GenCapacity reported for each
generation project.

During periods when projects are suspended, they cannot be operated and no
fixed O&M must be paid. However, capital recovery continues until the normal
end of life.

Plants that are suspended in one study period need not be suspended
in the next study period; this is a suspension, not a permanent retirement.
A permanent retirement requirement could be implemented by adding a side
constraint requiring m.SuspendGen[g, bld_yr, p] >= m.SuspendGen[g, bld_yr,
m.PERIODS.prev(p)].

TODO: Apply minimum build rules to retirements too.
"""
...
def define_components(mod):
    ...
    minus all suspensions and end-of-life retirements.
