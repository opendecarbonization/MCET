...
gis_cap_sf <- full_join(gis_sf, gen_cap_GW, by = c(region = "load_zone"),
...
### Flexible, allocation
```{r flex_load_alloc, fig.height=8, fig.cap="Shiftable load allocation, %"}
flex_baseload_alloc <- 
...
fig_flex_share <-
  ggplot() +
  geom_sf(data = gis_sf) +
  geom_sf(aes(fill = 100 * allocation), data = gis_flex_sf) +
...
gis_flex_gwh_sf <- gis_flex_sf |>
  rename(load_zone = load_zones) |>
  left_join(loads_y) |>
  mutate(allocation_GWh = allocation * zone_demand_GWh, 
         .after = "zone_demand_GWh")

fig_flex_TWh <-
  ggplot() +
  geom_sf(data = gis_sf) +
  geom_sf(aes(fill = allocation_GWh/1e3), data = gis_flex_gwh_sf) +
