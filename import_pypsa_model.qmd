...
format: 
  ...
  pdf:
    ...
    toc: true
execute:
  echo: true # should the code be echoed in the output document?
...
params: 
  ...
  ## 3. Update PyPSA-Earth dataset with MERRA2 data
  ...
  # YAML file parameters can be overwritten here
  # fig_map_width: 6
  # ...
---
...
if (nrow(network) > 0) {
  for (i in 1:nrow(network)) {
    trd <- newTrade(
      ...
      olife = 2, # doubled annualized invcost for consistency 
      stock = data.frame(
        # year = 
...
ror <- nc$generators_cf |>
  ...
  mutate( # !!! ToDo: write functions !!!
    # bus = str_extract(generators_t_p_max_pu_i, "[A-Z]+.[0-9]+"),
    bus = str_extract(generators_t_p_max_pu_i, " ror$"),
    tech_pypsa = str_trim(str_replace(generators_t_p_max_pu_i, bus, "")),
...
for (tch in unique(gen$tech_name)) {
  ...
  tech <- update(tech, stock = data.frame(
    year = p$base_year,
    region = x$region,
    stock = x$generators_p_nom
  ))
...
```{r}
#| eval: !expr params$build_switch
fl <- file.path(p$scen$BASE$switch, "modules.txt")
...
#   discount = p$discount,
#   slice = rev_data$timescales$energyRt_levels,
#   repository = repo
# )



# mod <- setTimeSlices(mod, slice = rev_data$timescales$energyRt_levels)
# mod@misc$gis <- gis
# mod@misc$plots <- list(
#   adm1_map = adm1_map,
...
# mod_path <- file.path(p$scen$BASE$root, "PyPSA_base_merra")
# mod_path <- p$scen$BASE$root
# dir.create(p$scen$BASE$root, showWarnings = F, recursive = T)
# save(mod, #network, voronoi_map, reg_nodes,
#      file = file.path(p$scen$BASE$root, "energyRt_model.RData"))

# save repositories
# ls(pattern = "repo_") # repositories
# save(list = ls(pattern = "repo_"), 
#      file = file.path(p$scen$BASE$root, "energyRt_repo.RData"))
...
# scen <- interpolate(mod, name = "PyPSA", trim = F)
# save(mod, file = file.path(p$scen$BASE$root, "energyRt_model.RData"))
scen_pypsa_base <- interpolate(mod, name = p$scen_name, trim = F)

# try(source("tmp/gams_config.R"))
source("tmp/solver_options.R")
scen_pypsa_base_gams <- write(scen_pypsa_base, 
                              solver = GAMS_parallel,
                              # solver = GAMS_mac,
                              tmp.dir = file.path(p$scen$BASE$root, "GAMS"))

scen_pypsa_base_gams@misc$tmp.dir 

solve(scen_pypsa_base_gams, wait = T)
scen_pypsa_base_gams <- read(scen_pypsa_base_gams)
summary(scen_pypsa_base_gams)
fname <- file.path(p$scen$BASE$root, glue("{p$scen_name}.RData"))
print(paste("Savinn:", fname, "\n"))
save(scen_pypsa_base_gams, file = fname)

# scen_PyPSA_Pyomo <- 
#   write(scen_PyPSA, solver = Pyomo, 
#         tmp.dir = file.path(p$dir$scen, "PyPSA", "Pyomo"))
# scen_PyPSA_Pyomo@misc$tmp.dir
# solve(scen_PyPSA_Pyomo, wait = F)

# scen_PyPSA_GAMS <- read(scen_PyPSA_GAMS)
# summary(scen_PyPSA_GAMS)
# save(scen_PyPSA_GAMS, file = file.path(p$dir$scen, "scen_PyPSA_GAMS.RData"))

# scen <- scen_pypsa_base_gams
# getData(scen, "vTechCap", merge = T, digits = 3) |> as.data.table()
# getData(scen, c("vTechNewCap", "vStorageNewCap"), merge = T, 
#         digits = 2, drop.zeros = T, process = T) |> as.data.table()
```
